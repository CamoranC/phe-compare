...
from lightphe.models.eek import eekinv
...
class RSA(Homomorphic):
        ...
        while True:
            try:
                while True:
                    ...
                    p = getPrime(ceildiv(key_size,2))#sympy.randprime(200, 2 ** int(key_size / 2) - 1)
                    q = getPrime(key_size // 2)#sympy.randprime(200, 2 ** int(key_size / 2) - 1)
                    if p != q:
                        break
                n = p * q
                phi = (p - 1) * (q - 1)
    ...
    def exponentiation_by_constant(self, ciphertext: int, constant: int) -> int:
        n = self.keys["public_key"]["n"]
        return pow(ciphertext,constant,n)

    def inv(self, ciphertext: int) -> int:
        n = self.keys["public_key"]["n"]
        return eekinv(ciphertext,n)